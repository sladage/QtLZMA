...
int QLZMA::encode(QByteArray &in, QByteArray &out)
{
    CLzmaEncHandle enc;
    ...
    if (res == SZ_OK)
    {
      ...
      if (outstream.writeRawData((char*)(&header), headerSize) != headerSize)
        res = SZ_ERROR_WRITE;
      else
      {
        if (res == SZ_OK)
...
static SRes Decode2(CLzmaDec *state, QDataStream* instream, QDataStream* outstream, quint64 uncompressed_size)
{
...
int QLZMA::decode(QByteArray &in, QByteArray &out, quint64 uncompressed_size)
{
    CLzmaDec state;
    QDataStream outstream(&out,QIODevice::WriteOnly);
    ...
    memcpy(&header,in.data(),LZMA_PROPS_SIZE);
    in.remove(0,LZMA_PROPS_SIZE);
    QDataStream instream(&in,QIODevice::ReadOnly);

    LzmaDec_Construct(&state);
    RINOK(LzmaDec_Allocate(&state, header, LZMA_PROPS_SIZE, &g_Alloc));

    SRes res = Decode2(&state, &instream, &outstream, uncompressed_size);
